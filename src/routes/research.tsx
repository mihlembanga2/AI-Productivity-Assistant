...
import { Search, Copy, Check, Sparkles, RefreshCw } from "lucide-react";
...
export const Route = createFileRoute("/research")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Get AI-powered research summaries and insights." },
    ],
  }),
...
function ResearchAssistant() {
  ...
  const handleResearch = async () => {
    ...
    setOutput("");
    try {
      const result = await research({ data: { topic, depth } });
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Get comprehensive research summaries with structured insights.</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Enter a topic to research.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  id="topic"
                  placeholder="e.g. Remote work productivity trends 2024"
                  value={topic}
              ...
              <div className="space-y-2">
                ...
                <Select value={depth} onValueChange={(v) => setDepth(v as typeof depth)}>
                  ...
                  <SelectContent>
                    <SelectItem value="overview">Overview — Key facts & concepts</SelectItem>
                    <SelectItem value="detailed">Detailed — Analysis with examples</SelectItem>
                    <SelectItem value="comprehensive">Comprehensive — Deep dive with trends</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="flex gap-2">
                <Button onClick={handleResearch} disabled={isLoading || !topic.trim()}>
                  {isLoading ? (
              ...
              <AnimatePresence mode="wait">
                {output ? (
                  ...
                  >
                    ...
                    <p className="text-sm">Enter a topic and start researching.</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </CardContent>
